...
public class MainAnalyzer {
    ...
    // 测试文件路径
    private static final String testFilePath = "test.txt";
    // 结果文件路径
    private static final String outFilePath = "resources/test_result.txt";
    // 类别码总数
    private static final int codeSum = 46;

    public static void main(String[] args) {
        String content = FileUtil.readFile(testFilePath);
        List<List<Token>> tokenQueueList = new ArrayList<>();
        for (int i = 0; i < codeSum; i++) {
            tokenQueueList.add(new ArrayList<>());
        }
        String result = scan(content, tokenQueueList);
        FileUtil.outputAsFile(outFilePath, result);
        System.out.println("-----词法分析完毕-----");
    }

    private static String scan(String content, List<List<Token>> tokenQueueList) {
        String res = "";
        int row = 1;  // 当前行数
        int col = 1;  // 当前列数
        int p = 0;    // 当前位指针
            ...
            if (c == '\n') {
                ...
                if (c == '*') {
                    ...
                    if (tmpP == content.length() - 1) {
                        ...
                        tokenQueueList.get(code).add(token);
                        res = res + token.toString() + '\n';
                        return res;
                    } else {
                ...
                if (code != -1) {
                    ...
                    res = res + "parse error: row " + row + ", col " + col + ", invalid syntax " + s + '\n';
                }
